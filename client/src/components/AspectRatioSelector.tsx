    ...
    <div className="space-y-3 dark">
      ...
      <div className="flex flex-wrap gap-2">
        {aspectRatios.map((ratio) => {
          ...
          return(
            <button key={ratio} type="button" className={`flex items-center gap-2 rounded-md border px-5 py-2.5 text-sm transition border-white/10 ${selected?"bg-white/10": "hover:bg-white/6"}`} onClick={()=>onChange(ratio)}>
