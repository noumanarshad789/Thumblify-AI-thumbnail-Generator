  ...
  return (
    <div className="relative max-auto w-full max-w-2xl">
      <div className={`relative overflow-hidden ${aspectClasses[aspectRatio]}`}>
        ...
        {!isLoading && thumbnail?.image_url && (
            <div className="group relative h-full w-full">
                ...
                <div className="absolute inset-0 flex items-end justify-center bg-black/10 opacity-0 transition-opacity group-hover:opacity-100">
                    <button type="button" onClick={onDownload} className="mb-6 flex items-center gap-2 rounded-md px-5 py-2.5 text-xs font-medium transition bg-white/30 ring-2 ring-white/40 backdrop-blur hover:scale-105 active:scale-95">
        ...
        {/* Empty State */}
        {!isLoading && !thumbnail?.image_url &&(
          <div className="absolute inset-0 m-0 flex flex-col items-center justify-center gap-4 rounded-lg border-2 border-dashed border-white/20 bg-black/25">
